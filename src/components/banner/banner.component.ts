import { Component, OnInit, ViewEncapsulation, ViewChild, ElementRef, ApplicationRef } from '@angular/core';
...
export class BannerComponent implements OnInit {
  ...
  @ViewChild("bannerslide", { read: ElementRef, static: true }) bannerslide: ElementRef;
  ...
  slideChange(event) {
    console.log('banner changes')
    this.bannerslide.nativeElement.getActiveIndex().then(index => {
      setTimeout(() => {
        if (index == this.shared.banners.length - 1) this.bannerslide.nativeElement.slideTo(0, 1000)
        else this.bannerslide.nativeElement.slideTo(index + 1, 1000)
        this.bannerslide.nativeElement.loop = true
        this.bannerslide.nativeElement.startAutoplay().then(() => { })
  ...
  getSingleProductDetail(id) {
    ...
    this.config.postHttp(this.config.url + 'getallproducts', dat).then((data: any) => {
      console.log('getallproducts 1 == ', dat)
      this.loading.hide();
      if (data.success == 1) {
